...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var skipCountdown:FlxUICheckBox;
	var startAtTime:FlxUICheckBox;
	...
	function addChartingUI() {
		...
		skipCountdown = new FlxUICheckBox(120, 190, null, null, "Skip Countdown", 80,
			function() {
				FlxG.save.data.chart_skipCountdown = skipCountdown.checked;
			}
		);
		if (FlxG.save.data.chart_skipCountdown == null) FlxG.save.data.chart_skipCountdown = false;
		skipCountdown.checked = FlxG.save.data.chart_skipCountdown;


		startAtTime = new FlxUICheckBox(230, 190, null, null, "Skip to time", 80,
			function() {
				FlxG.save.data.chart_startAtTime = startAtTime.checked;
			}
		);
		if (FlxG.save.data.chart_startAtTime == null) FlxG.save.data.chart_startAtTime = false;
		startAtTime.checked = FlxG.save.data.chart_startAtTime;
		...
		tab_group_chart.add(skipCountdown);
		tab_group_chart.add(startAtTime);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			else if (FlxG.keys.justPressed.ENTER)
			{
				...
				PlayState.startOnTime = songStartTime;
				PlayState.forceSkipCountDown = skipCountdown.checked;
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				PlayState.forceSkipCountDown = false;
		...
		songStartTime = startAtTime.checked ? lastConductorPos : 0;
